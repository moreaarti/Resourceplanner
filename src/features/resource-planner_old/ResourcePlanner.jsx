...
const Calendar = () => {
  ...
  async function fetchData() {
    ...
    console.log("llllllllllllfetchdata response", response);
    console.log("usersvvvvvvvvvv", users);
    let allAssignments = [];
    ...
    users.forEach((user) => {
      ...
      assignments.forEach((assignment) => {
        allAssignments.push({
          ...
          end: assignment.to_date || "2026-010-01",
  ...
  //   try {
  //     const response = await getResourcePlannerApi(
  //       companyID,
  ...
  //         allAssignments.push({
  //           id: assignment.id,
  //           title: assignment.project_name || "",
  //           start: assignment.from_date || "2025-06-01",
  //           end: assignment.to_date || "2025-07-01",
  //           resourceId: assignment.user_id,
  //           assignment_type: assignment.assignment_type,
  //         });
  //       });
  ...
  const addItemHandler = ({ taskProject, member, start, end, notes }) => {
    ...
    console.log("adddddd events", events);
    setOpen(false);
  };
        ...
        <div className="resource-planner">
          <FullCalendar
            ...
            resourceLabelContent={(arg) => {
              ...
              const handleClick = () => {
                const clickedUserId = Number(resource.id); // "491" → 491

                // Match the user from API response
                const selectedUser = usersList.find(
                  (u) => u.userId === clickedUserId,
                );

                if (selectedUser) {
                  console.log(
                    "Selected user from resource match:",
                    selectedUser,
                  );
                  setSelectedUser(selectedUser); // Set for form prefill
                  setOpen(true);
                } else {
                  console.warn(
                    "No matching user found in usersList for resource id",
                    resource.id,
                  );
                }
              };
