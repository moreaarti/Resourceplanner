...
export async function deleteResourcePlannerApi(delete_id) {
  ...
  const graphqlQuery = {
    query: `
      mutation {
        deleteEmployeeAssignment(input: { id: ${delete_id} }) {
          success
          message
        }
      }
...
export const getTasksListforDD = async (
  ...
  try {
    ...
    // console.log("res in res plan api file", res?.data?.data.projectTasks);
    if (res?.data?.errors) {
      tokenExpireLogoutHandler(res?.data?.errors?.[0]?.message);
      return null;
    }
...
export const getProjectsListforDD = async (
  ...
  const graphqlQuery = {
    query: `
      query {
        projects(where: {search: "${search}"}, first: ${first}, after: "${after}") {
          nodes {
            id
            projectId
            title
...
export async function addTaskApi(fields) {
  ...
  //     mutation CreateProjectTask {
  //   createProjectTask(
  //     input: {clientMutationId: "CreateProjectTask",
  //     title: "Task 1",

  //     startDate: "2025-06-29",
  //      dueDate: "2025-06-30",
  //      status: PUBLISH, assignedTo: [],
  //      estimate: 10,
  //       taskStatues: {nodes: [{id: "dGVybTo4Mw=="}]},

  //         relatedProject: 10687,
  //      }
  //   ) {
  //     projectTask {
  //       id
  ...
  const formatNodes = (nodes) =>
    nodes && nodes.length
      ? `{ nodes: [${nodes.map((node) => `{ id: "${node.id}" }`).join(", ")}] }`
      : "";
  const formatArray = (arr) =>
    Array.isArray(arr) ? `[${arr.map((v) => `"${v}"`).join(", ")}]` : "[]";
  // const graphqlQuery = {
  //   query: `
  //     mutation CreateProjectTask {
  //       createProjectTask(
  ...
  //           dueDate: "${fields.end || ""}",
  //           status: PUBLISH,
  //           assignedTo: ${formatArray(fields.member || [])},
  //           estimate: ${fields.totalHours || 0},
  //           taskStatues: ${formatNodes(fields.taskStatues?.nodes || [])},
  //           relatedProject: ${11004 || null}
  //         }
  //       ) {
  //         projectTask {
  //           id
  ...
  createProjectTask(
    input: {clientMutationId: "CreateProjectTask", title:"${
      ...
      fields.member || ""
    }",, estimate: 10, taskStatues: {nodes: [{id: "dGVybTo4Mw=="}]}, taskTags: {nodes: [{id: "dGVybTo4Mw=="}]}, relatedProject: 10687}
  ) {
    projectTask {
      id
    }
  ...
  console.log("graphqlQuery for add", graphqlQuery);
  try {
    const res = await axios.post(config.API_URL, graphqlQuery, {
      headers: {
        Authorization: `Bearer ${token_value}`,
