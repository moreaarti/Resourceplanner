...
const ProjectsTaskTable = (props) => {
  
  const page_Loader_data = useLoaderData();
  const current_workspace_id = page_Loader_data?.params?.workspace_id;
  const [params]=useSearchParams();
  const singleWorkspaceId=params.get("workspace_id");
  const setting_current_workspace_id = useSelector(state => state.workspace.current_workspaceId);
  const site_id = page_Loader_data?.params?.site_id;
  ...
  return (
    <div className="custom-table relative  ">
      <DataTable
        ...
        expandableRows
        expandableRowExpanded={(row) => row.tasks && row.tasks.length > 0}
        expandableRowsComponent={({ data }) => (
          // console.log("daaaaaaaaa",data)
          <div className="mb-2">
            {data.tasks.map(task => (
              <div key={task.key} className="p-2 border-b">
                {task.task_name}
              </div>
            ))}
          </div>
...
export default ProjectsTaskTable;
