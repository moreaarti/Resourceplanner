...
export default function EmployeeDropDownNew({
  ...
  useEffect(() => {
    let getUserDetails = employee_dropdown_data_list.filter(
      (o) => o.userId === parseInt(defaultValue),
    );
    getUserDetails = getUserDetails.length > 0 ? getUserDetails[0] : {};
    getUserDetails = {
      id: getUserDetails?.userId || "",
      name: getUserDetails?.firstName + " " + getUserDetails?.lastName || "",
      profileImage: getUserDetails?.profileImage || "",
      userDesignation: getUserDetails?.userDesignation || "",
    };
    setCurrentValue(getUserDetails);
