...
const Calendar = () => {
  ...
  const customTaskRender = (eventInfo) => {
    ...
    const assignmentType = eventInfo.event.extendedProps.assignment_type;
    const leave_type1 = eventInfo.event.extendedProps.leave_type;
    const created_by = eventInfo.event.extendedProps.created_by;
    const totalHours = eventInfo.event.extendedProps.created_by;
    const totalTrackedTime = eventInfo.event.extendedProps.total_tracked_time;
    console.log("totalHours", totalHours);
    const item = {
      ...eventInfo.event.extendedProps,
      start: eventInfo.event.start,
      totalHours: eventInfo.event.totalHours,
      // start: eventInfo.event.start || new Date(eventInfo.event.startStr),
      end: eventInfo.event.end ? new Date(eventInfo.event.end) : null,
      // end: eventInfo.event.end || new Date(eventInfo.event.endStr),
      assignment_type: assignmentType,
      total_tracked_time: totalTrackedTime,
      task: title,
      leave_type: leave_type1,
