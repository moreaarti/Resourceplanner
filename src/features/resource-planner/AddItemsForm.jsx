...
// import { addTaskApi } from "./resourcesPlannerApi";
import {
  errorMessageHandler,
...
const AddItemsForm = ({
  ...
  console.log("selectedUser 11", selectedUser);
  ...
  const [member, setMember] = useState(selectedUser || "");

  const [hoursDays, setHoursDays] = useState(8); //    hr/day
  const [totalHours, setTotalHours] = useState(8); //total
...
`;
  ...
  // close this popup when user click outside or if they press escape key, close the popup
  useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      if (open && event.target.closest(".add-item-form") === null) {
        setOpen(false);
      }
    };
  ...
  useEffect(() => {
    const calculateTotalHours = () => {
      if (start && end && hoursDays) {
        ...
        const hoursPerDay = parseFloat(hoursDays);
        const total = durationInDays * hoursPerDay;

        setTotalHours(Number(total.toFixed(0)));
      }
    };
  ...
  const handleSave = async (e) => {
    ...
    const fields = {
      ...
      notes: notes,
      notifyTo: 0,
      estimate: parseInt(totalHours) || 0,
      relatedProject: projectList.projectId, //need to changeeeeeeeeeeeeeeeeprojectList
    };
    console.log("fields in handle save", fields);
    // === Validation Checks ===
    if (!fields.start) {
    ...
    if (!fields.end) {
      errorMessageHandler(enqueueSnackbar, "Please select an end date.");
      return;
    }

    if (!fields.taskName || fields.taskName.trim() === "") {
      errorMessageHandler(enqueueSnackbar, "Please Enter a task.");
      return;
    }

    if (!fields.member || !fields.member.length || isNaN(fields.member[0])) {
      errorMessageHandler(enqueueSnackbar, "Please select a task member.");
      return;
    }
    ...
    try {
      ...
      if (response?.data?.errors) {
        errorMessageHandler(
          enqueueSnackbar,
          response.data.errors[0]?.message || "Error while adding task",
        );
        return;
      }
      ...
      errorMessageHandler(enqueueSnackbar, "Something went wrong.");
    }
  };
  ...
  return (
    <>
      {open && (
        <div className=" justify-cente add-item-form flex h-[800.27px] items-center">
          <div className="w-[600px] rounded-lg border bg-white">
            ...
            <div className="flex flex-col gap-[26px] ">
              <div className="">
                ...
                {activeTab === "task" && (
                  <>
                    <div className="scrollbar-thin mt-[26px] flex h-[200px] flex-col gap-[26px] overflow-hidden overflow-y-auto px-7 2xl:h-[350px] min-[1700px]:h-full">
                      ...
                      <GetProjectListDD
                        ...
                        projectMember={selectedUser}
                      />

                      {/* <div className="flex flex-col gap-[10px] px-7"> */}
                      <label
                        htmlFor="task-project"
                        className="mb-0 text-base leading-[100%]"
                      >
                        Enter Task
                      </label>
                      <input
                      ...
                      <div className="flex flex-col gap-[10px]">
                        <EmployeeDropDownNew
                          ...
                          required={true}
                          defaultValue={member}
                          // managerHandler={(item) => setMember(item)}
