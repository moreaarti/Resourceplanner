...
import {
  ...
  setSingleEmployeeAttachmentsView,
  setSingleEmployeeView,
  setUserData,
} from "../hr/emplyee/employeeSlice";
import {
  setCountryData,
  setEmployeeDropDownList,
  setEmployeeDropDownListHasNext,
  setEmployeeDropDownPagination,
  setEmployeeDropDownSearchList,
...
export async function getEmployeeData(
  ...
  const status_value =
    statusValue === "Active" || statusValue === undefined ? false : true;
  const gender_value =
    genderValue === "All" || genderValue === undefined ? "" : genderValue;
  const employment_value =
...
export async function getSingleEmployeeAttachmentsData(user_id) {
  ...
  if (res?.data?.data?.mediaItems) {
    store.dispatch(
      setSingleEmployeeAttachmentsView(res?.data?.data?.mediaItems),
    );
    return res;
  }
}
...
export async function getEmployeeDropdownList(
  companyId = null,
  ...
  if (res?.data?.data?.users?.nodes) {
    store.dispatch(
      setEmployeeDropDownPagination(res?.data?.data?.users?.pageInfo),
    );
    ...
    if (getHasNext) {
      store.dispatch(
        setEmployeeDropDownListHasNext(res?.data?.data?.users?.nodes),
      );
    }
    if (getSearch) {
      store.dispatch(
        setEmployeeDropDownSearchList(res?.data?.data?.users?.nodes),
      );
    }
  }
  return res;
...
export async function updatePreviousEmploymentInformationApi(
  id,
  previous_employeement,
) {
  const userData = token();
  const token_value = userData?.authToken;
