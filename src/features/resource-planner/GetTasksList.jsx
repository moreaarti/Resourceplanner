...
export default function GetTasksListDD({
  ...
  const { resourceTasks, resourceTasksPagination } = useSelector(
    (state) => state.resourcesPlanner,
  );
  ...
  const loadInitialData = async () => {
    ...
    dispatch(setResourceTasks(res.nodes));
    dispatch(setResourceTasksPagination(res.pageInfo));
  };
  ...
  useEffect(() => {
    if (projectId) {
      ...
      dispatch(setResourceTasks([]));
      dispatch(setResourceTasksPagination({}));
  ...
  const loadMoreHandler = async () => {
    ...
    const res = await getTasksListforDD(
      searchValue,
      10,
      resourceTasksPagination?.endCursor,
      projectId,
    );
      ...
      {dropdownOpen && (
        ...
        >
          ...
          <div className="max-h-[200px] overflow-y-auto">
            ...
            {resourceTasksPagination?.hasNextPage && (
